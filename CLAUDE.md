...
| `model_gallery.py` | Node + HTTP endpoints (`/list` incl. size, `/meta` incl. dtype/params/size + LoRA triggers/topology/training, `/hash` on-demand checksum, `/thumb` stub). Bundled libs only; arbitrary-path endpoints gate on an extension whitelist; `/meta` + `/hash` resolve paths only via `folder_paths`. |
